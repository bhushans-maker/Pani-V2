...
export default function App() {
  ...
  // ── Poll native alarm status every 2s (catches alarm when app was killed) ──
  useEffect(() => {
    if (!isNative()) return;
    ...
    const pollInterval = setInterval(async () => {
      if (useStore.getState().isAlarmRinging) return; // Already ringing
  ...
  useEffect(() => {
    ...
    const requestPerms = async () => {
      ...
      if (!overlayOk) {
        alert("AquaAlarm needs 'Display over other apps' to show the alarm screen. Please grant it.");
        await alarmNativeManager.requestOverlayPermission();
      }
      const batteryOk = await alarmNativeManager.checkBatteryPermission();
      if (!batteryOk) {
        alert("Please disable battery optimization for AquaAlarm so alarms fire reliably.");
  ...
  useEffect(() => {
    ...
    const listener = CapApp.addListener('backButton', () => {
      if (isAlarmRinging || isAlarmUIVisible) return; // Block during alarm
  ...
  // ── Polling fallback when app is open in foreground ────────────────────────
  useEffect(() => {
    if (!profile || !isAlarmEnabled) return;
    const iv = setInterval(() => {
      const next = getNextAlarmTime();
      if (next && Date.now() >= next && !useStore.getState().isAlarmRinging) {
        setAlarmRinging(true);
      }
    }, 3000);
    return () => clearInterval(iv);
  }, [profile, isAlarmEnabled, getNextAlarmTime, setAlarmRinging]);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col max-w-md mx-auto relative shadow-2xl overflow-hidden">
      ...
      {/* Alarm overlay — blocks everything until user goes to dashboard and logs water */}
      <AnimatePresence>
        {isAlarmUIVisible && (
          <motion.div
            initial={{ opacity: 0 }}
